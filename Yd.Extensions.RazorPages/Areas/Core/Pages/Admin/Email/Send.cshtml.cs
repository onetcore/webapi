...
namespace Yd.Extensions.RazorPages.Areas.Core.Pages.Admin.Email
{
    ...
    public class SendModel : ModelBase
    {
        ...
        {
            var message = _messageManager.Find(id) ?? new Gentings.Messages.Emails.Email();
        ...
        public IActionResult OnPost()
        {
            if (!ModelState.IsValid)
                return Error();
            Gentings.Messages.Emails.Email message;
            if (Input.Id > 0)
            {
            ...
            message = new Gentings.Messages.Emails.Email
            {
                UserId = UserId,
            };
            message.Title = Input.Title;
