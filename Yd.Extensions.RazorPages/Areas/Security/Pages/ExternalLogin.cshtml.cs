...
namespace Yd.Extensions.RazorPages.Areas.Security.Pages
{
    ...
    public class ExternalLoginModel : ModelBase
    {
        ...
        public IActionResult OnPost(string provider, string returnUrl = null)
        {
            ...
            var redirectUrl = UrlHelperExtensions.Page(Url, "./ExternalLogin", pageHandler: "Callback", values: new { returnUrl });
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (result.Succeeded)
            {
                Log($"{info.Principal.Identity.Name} 通过 {info.LoginProvider} 进行登录。");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        Log("用户通过 {0} 新注册了一个用户。", info.LoginProvider);
                        return LocalRedirect(returnUrl);
                    }
                }
