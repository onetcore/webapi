...
using Yd.WebUI.Core;
using Yd.WebUI.UI.StatusMessages;

namespace Yd.WebUI.UI
{
    /// <summary>
    /// 页面模型基类。
    /// </summary>
    public abstract class ModelBase : PageModel
    {
        ...
        /// <returns>返回当前页面结果。</returns>
        protected IActionResult Page(ApiResult result, params object[] args)
        {
            if (result.Status)
                return Page(StatusType.Success, string.Format(result.Message, args));
            return Page(StatusType.Danger, string.Format(result.Message, args));
        }
        ...
        /// <param name="args">参数。</param>
        /// <returns>返回当前页面结果。</returns>
        protected IActionResult RedirectToResult(ApiResult result, params object[] args)
        {
            if (result.Status)
                return RedirectToPage(StatusType.Success, string.Format(result.Message, args));
            return RedirectToPage(StatusType.Danger, string.Format(result.Message, args));
        }
        #endregion
        ...
        protected IActionResult Json(ApiResult result, params object[] args)
        {
            if (result.Status)
                return Json(StatusType.Success, string.Format(result.Message, args));
            return Json(StatusType.Danger, string.Format(result.Message, args));
        }
